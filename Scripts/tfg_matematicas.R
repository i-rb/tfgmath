# -*- coding: utf-8 -*-
"""TFG_matematicas.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1X3CjvW2J2UDByIl__xd48oDYboMA2ouQ

## TFG Matemáticas

## 0 Import Libraries & Data

Librerías utilizadas:
* ggplot2

Primero, importamos los datos sobre los que queremos trabajar. En nuestro caso, y dado lo adeucado de este tipo de análisis sobre series financieras, he decido utilizar el histórico (desde 1960) hasta hoy (2020) diario del valor de la acción de IBM en la bolsa de Nueva York (N

_A día 21 de mayo del 2020, tarda en:_
- _instalar paquetes: $\approx 15\mathrm{min}$_
- _cargar el resto del cuaderno: prácticamente nada
"""

# primero se instala la TSA desde github (ahora fuera de CRAN)
Sys.time()
devtools::install_github("cran/TSA")

Sys.time()
# not-installed libs 
print(" ")
print("Installing dygraphs")
install.packages("dygraphs")
print(" ")
print("Installing fNonlinear")
install.packages("fNonlinear")
print(" ")
print("Installing tseries")
install.packages("tseries")
print(" ")
print("Installing forecast")
install.packages("forecast")
print(" ")
print("Installing rugarch")
install.packages("rugarch")
print(" ")
print("Installing fRegression")
install.packages("fRegression")
print(" ")
print("Installing tseriesChaos")
install.packages("tseriesChaos")
print(" ")
print("Installing Chaos01")
install.packages("Chaos01")
print(" ")
print("Installing pracma")
install.packages("pracma")
print(" ")
print("Installing DChaos")
install.packages("DChaos")
print(" ")
print("Installing nonlinearTseries")
install.packages("nonlinearTseries")
print(" ")
print("Installing fractal")
install.packages("fractal")
print(" ")
print("Installing liftLRD")
install.packages("liftLRD")
Sys.time()

install.packages("liftLRD")

# libs # hay que ejecutarlo 2 veces, creo poder arreglarlo con excepciones (trycatch)
Sys.time()
library(pracma)
library(ggplot2)
library(dygraphs)
library(lubridate)
library(xts)          # To make the convertion data-frame / xts format
library(tidyverse)
library(htmlwidgets)
library(fNonlinear)
library(tseries)
library(forecast)
library(fRegression)
library(lmtest)
library(TSA)
library(rugarch)
library(Chaos01)
library(tseriesChaos)
library(DChaos)
library(nonlinearTseries)
library(fractal)
library(liftLRD)

#  Es necesario subir IBM.csv con anterioridad!! (o añadirlo a la carpeta si es un entorno local)
ibmdata = read.csv("IBM.csv",sep=",") # read csv 
ibmdata$Date <- as.Date(ibmdata$Date,format="%Y-%m-%d") # date formatting

head(ibmdata)

dd <- xts(x = ibmdata$Close, order.by = ibmdata$Date)

p <- dygraph(dd) %>%
  dyOptions(labelsUTC = TRUE, fillGraph=TRUE, fillAlpha=0.2, drawGrid = FALSE, colors="#0000FF") %>%
  dyRangeSelector() %>%
  dyCrosshair(direction = "vertical") %>%
  dyHighlight(highlightCircleSize = 5, highlightSeriesBackgroundAlpha = 0.2, hideOnMouseOut = FALSE)  %>%
  dyRoller(rollPeriod = 1)

# saveWidget(p,"ibm_stocks.html") # guardamos el gráfico interactivo. Aquí abrimos
# una pequeña representación: (que también guardamos)

p <- ggplot(ibmdata, aes(x=Date, y=Close)) +
  geom_line() + 
  xlab("")
p
ggsave(file="ibm_serie.eps")

"""### 0.1 Estadísticas Descriptivas de la serie original

Media, Mediana, DT, Mínimo, Máximo, Asimetría, Curtosis, Jarque-Bera y N
"""

y=ibmdata$Close

mean(y)

median(y)

sd(y)

min(y)

max(y)

skewness=function(x) {
m3=mean((x-mean(x))^3)
skew=m3/(sd(x)^3)
skew}
skewness(y)

kurtosis=function(x) {
m4=mean((x-mean(x))^4)
kurt=m4/(sd(x)^4)-3 
kurt}
kurtosis(y)

jarque.bera.test(y)

length(y)

serie_original=y
plot=qplot(serie_original, geom="histogram") 
plot
ggsave(plot,file="hist_original.eps")



"""### 0.2 Creación d_ln_y_t (y estadísticas descriptivas)

Se crea la variable $z_t = \nabla\ln (y_t)$
"""

ln_diff=c(NA,diff(log(ibmdata$Close)))
ibmdata$d_ln_close=ln_diff #creamos la variable d_ln_y (1st row, NA)

head(ibmdata)

y=ibmdata$d_ln_close

y=y[2:length(y)]
red=ibmdata[2:nrow(ibmdata),]

p <- ggplot(red, aes(x=Date, y=d_ln_close)) +
  geom_line() + 
  xlab("")
p
ggsave(file="ibm_retornos.eps")

mean(y)

median(y)

sd(y)

min(y)

max(y)

skewness=function(x) {
m3=mean((x-mean(x))^3)
skew=m3/(sd(x)^3)
skew}
skewness(y)

kurtosis=function(x) {
m4=mean((x-mean(x))^4)
kurt=m4/(sd(x)^4)-3 
kurt}
kurtosis(y)

jarque.bera.test(y)

length(y)

retornos=y
plot=qplot(retornos, geom="histogram") 
plot
ggsave(plot,file="hist_retornos.eps")

qqnorm(y)
pl=qqline(y, col = "blue")
ggsave(pl,file="qqplot_retornos.eps")

m <- lm(y ~ 1)
for (i in 1:8){print(bgtest(m,order=i))}

"""## 1 Modelización de la serie

### 1.1 Modelización ARMA
"""

p=plot(acf(y,plot=F)[1:35])
ggsave(p,file="ACF_retornos.eps")

p=plot(pacf(y,plot=F)[0:35])
ggsave(p,file="PACF_retornos.eps")

"""**ARMA(0,0)**"""

auto.arima(y,ic="bic")

arma00=arima(y,c(0,0,0))

bic=AIC(arma00,k=log(length(y)))
bic

"""**ARMA(1,1)**"""

arma11=arima(y,c(1,0,1))

bic=AIC(arma11,k = log(length(y)))
bic

residuos_arma11=residuals(arma11)

arma11

coeftest(arma11)

p=plot(acf(residuos_arma11,plot=F)[1:35])
ggsave(p,file="ACF_arma11.eps")

p=plot(pacf(residuos_arma11,plot=F)[0:35])
ggsave(p,file="PACF_arma11.eps")

"""#### Estadísticas Descriptivas de los residuos del ARMA(1,1)

Media, Mediana, DT, Mínimo, Máximo, Asimetría, Curtosis, Jarque-Bera y N
"""

y=residuos_arma11

mean(y)

median(y)

sd(y)

min(y)

max(y)

skewness=function(x) {
m3=mean((x-mean(x))^3)
skew=m3/(sd(x)^3)
skew}
skewness(y)

kurtosis=function(x) {
m4=mean((x-mean(x))^4)
kurt=m4/(sd(x)^4)-3 
kurt}
kurtosis(y)

jarque.bera.test(y)

length(y)

residuos_arma11=y
plot=qplot(residuos_arma11, geom="histogram") 
plot
ggsave(plot,file="hist_arma11.eps")

qqnorm(y)
pl=qqline(y, col = "blue")
ggsave(pl,file="qqplot_arma11.eps")

plot(y)

#test heterocedasticidad
m <- lm(y ~ 1)
for (i in 1:8){print(bgtest(m,order=i))}

"""### Tests de raíz unitaria y estacionariedad"""

y=ibmdata$d_ln_close[2:nrow(ibmdata)]
x=ibmdata$Close

adf.test(x)

adf.test(y)

pp.test(x) #phillips perron

pp.test(y)

kpss.test(x)

kpss.test(y)

"""### 1.2 Modelización GARCH

**GARCH(1,1)**
"""

y=ibmdata$d_ln_close
y=y[2:length(y)]

garch11=garch(y,c(1,1))

bic=AIC(garch11,k = log(length(y)))
bic

garch11

coeftest(garch11)

residuos_garch11=residuals(garch11)

#for (i in 0:3){
#  for (j in 1:3){
#    mod <- garch(y,c(i,j))
#    bic=AIC(mod,k = log(length(y)))
#    print(c('----->',i,j,":",bic))
#  }
#}

residuos_garch11=residuals(garch11)[2:length(residuos_garch11)]

"""#### Estadísticas Descriptivas de GARCH(1,1)

Media, Mediana, DT, Mínimo, Máximo, Asimetría, Curtosis, Jarque-Bera y N
"""

y=residuos_garch11

mean(y)

median(y)

sd(y)

min(y)

max(y)

skewness=function(x) {
m3=mean((x-mean(x))^3)
skew=m3/(sd(x)^3)
skew}
skewness(y)

kurtosis=function(x) {
m4=mean((x-mean(x))^4)
kurt=m4/(sd(x)^4)-3 
kurt}
kurtosis(y)

jarque.bera.test(y)

length(y)

serie_garch11=y
plot=qplot(serie_garch11, geom="histogram") 
plot
ggsave(plot,file="hist_garch11.eps")

qqnorm(y)
pl=qqline(y, col = "blue")
ggsave(pl,file="qqplot_garch11.eps")

plot(y,type="l")

#test heterocedasticidad
m <- lm(residuals(garch11) ~ 1)
for (i in 1:8){print(bgtest(m,order=i))}

"""### 1.3 Modelización EGARCH"""

y=ibmdata$d_ln_close

y=y[2:length(y)]

spec = ugarchspec(variance.model = list(model = "eGARCH", garchOrder = c(2,1)),mean.model=list(armaOrder=c(0,0),include.mean=FALSE))
earch11 = ugarchfit(y, spec = spec)

residuos_earch11=residuals(earch11,standardize=TRUE) # se estandarizan los residuos

earch11

"""#### Estadísticas Descriptivas de EGARCH(2,1)

1.   Elemento de lista
2.   Elemento de lista



Media, Mediana, DT, Mínimo, Máximo, Asimetría, Curtosis, Jarque-Bera y N
"""

y=residuos_earch11

mean(y)

median(y)

sd(y)

min(y)

max(y)

skewness=function(x) {
m3=mean((x-mean(x))^3)
skew=m3/(sd(x)^3)
skew}
skewness(y)

kurtosis=function(x) {
m4=mean((x-mean(x))^4)
kurt=m4/(sd(x)^4)-3 
kurt}
kurtosis(y)

jarque.bera.test(y)

length(y)

serie_egarch21=y
plot=qplot(serie_egarch21, geom="histogram") 
plot
ggsave(plot,file="hist_earch11.eps")

qqnorm(y)
pl=qqline(y, col = "blue")
ggsave(pl,file="qqplot_eearch11.png")

plot(y)

#test heterocedasticidad
m <- lm(y ~ 1)
for (i in 1:8){print(bgtest(m,order=i))}

















Sys.time()

"""## 2 Tests de linealidad"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

"""### 2.1. BDS Test"""

bds.test(x, m = 10, eps = seq(0.5 * sd(x), 2 * sd(x), length = 4),
         trace = FALSE)

bds.test(y, m = 10, eps = seq(0.5 * sd(y), 2 * sd(y), length = 4),
         trace = FALSE)

bds.test(z, m = 10, eps = seq(0.5 * sd(z), 2 * sd(z), length = 4),
         trace = FALSE)

bds.test(w, m = 10, eps = seq(0.5 * sd(w), 2 * sd(w), length = 4),
         trace = FALSE)



"""### 2.2 Test de Keenan"""

Keenan.test(x,1)

Keenan.test(y,1)

Keenan.test(z,1)

Keenan.test(w,1)

"""### 2.3 Test de Tsay"""

Tsay.test(x,1)

Tsay.test(y,1)

Tsay.test(z,1)

Tsay.test(w,1)



"""### 2.4 Test de White"""

white.test(x)

white.test(y)

white.test(z)

white.test(w)

"""### 2.5 Test de Terasvirta"""

terasvirta.test(x)

terasvirta.test(y)

terasvirta.test(z)

terasvirta.test(w)

"""### 2.6 Test RESET de Ramsey"""

m <- lm(x ~ 1)
resetTest(m)

m <- lm(y ~ 1)
resetTest(m)

m <- lm(z ~ 1)
resetTest(m)

m <- lm(w ~ 1)
resetTest(m)

"""## 3 Test de Caos

### 3.1 Gráficos de recurrencia
"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

fNonlinear::recurrencePlot(x,m=1,d=1,eps=0.01,end.time=5001,pch = ".")

fNonlinear::recurrencePlot(y,m=1,d=1,eps=0.01,end.time=5001,pch = ".")

fNonlinear::recurrencePlot(z,m=1,d=1,eps=0.7,end.time=5001,pch = ".")

fNonlinear::recurrencePlot(w,m=1,d=1,eps=0.7,end.time=5001,pch = ".")

#######

fNonlinear::recurrencePlot(x,m=2,d=2,eps=0.01,end.time=5001,pch = ".")

fNonlinear::recurrencePlot(y,m=2,d=2,eps=0.01,end.time=5001,pch = ".")

fNonlinear::recurrencePlot(z,m=2,d=2,eps=0.7,end.time=5001,pch = ".")

fNonlinear::recurrencePlot(w,m=2,d=2,eps=0.7,end.time=5001,pch = ".")

#########

recurr(lorenz.ts, m=3, d=2, start.time=15, end.time=30)

#####

recurr(x, m=2, d=1, start.time=350, end.time=500)

recurr(y, m=2, d=1, start.time=350, end.time=500)

recurr(z, m=2, d=1, start.time=350, end.time=500)

recurr(w, m=2, d=1, start.time=350, end.time=500)

"""### 3.2 Test 0-1"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

testChaos01(x)

testChaos01(y)

testChaos01(z)

testChaos01(w)

"""### 3.3. Coeficiente de Hurst"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

hurstexp(x)

hurstexp(y)

hurstexp(z)

hurstexp(w)

liftHurst(x[(length(x)-500):length(x)]) # computacionalmente es demasiado, se acorta la serie a las ultimas 500 observaciones

liftHurst(y[(length(y)-500):length(y)]) # computacionalmente es demasiado, se acorta la serie a las ultimas 500 observaciones

liftHurst(z[(length(z)-500):length(z)]) # computacionalmente es demasiado, se acorta la serie a las ultimas 500 observaciones

liftHurst(w[(length(w)-500):length(w)]) # computacionalmente es demasiado, se acorta la serie a las ultimas 500 observaciones

"""### 3.4. Dimensión de correlación"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=as.numeric(residuos_earch11)

corrDim(x)

corrDim(y)

corrDim(z)

corrDim(w)

"""### 3.5 Mapa del espacio de fases"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

takens = buildTakens( x, embedding.dim=2, time.lag=1)
plot(takens, pch='.', type="l")

takens = buildTakens( y, embedding.dim=2, time.lag=1)
plot(takens, pch='.',type="l")

takens = buildTakens( z, embedding.dim=2, time.lag=1)
plot(takens, pch='.',type="l")

takens = buildTakens( w, embedding.dim=2, time.lag=1)
plot(takens, pch='.',type="l")

"""### 3.6 Algoritmo MESAH"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

ye=x
MAX=max(ye)
MIN=min(ye)
y_1=ye[1:(length(ye)-1)]
ye=ye[2:length(ye)]
value=((ye-MAX)^2+(y_1-MIN)^2)
value=value/2
hist(value)

ye=y
MAX=max(ye)
MIN=min(ye)
y_1=ye[1:(length(ye)-1)]
ye=ye[2:length(ye)]
value=((ye-MAX)^2+(y_1-MIN)^2)
value=value/2
hist(value)

ye=z
MAX=max(ye)
MIN=min(ye)
y_1=ye[1:(length(ye)-1)]
ye=ye[2:length(ye)]
value=((ye-MAX)^2+(y_1-MIN)^2)
value=value/2
hist(value)

ye=w
MAX=max(ye)
MIN=min(ye)
y_1=ye[1:(length(ye)-1)]
ye=ye[2:length(ye)]
value=((ye-MAX)^2+(y_1-MIN)^2)
value=value/2
hist(value)

"""### 3.7 Mapa de Poincaré"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z0=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

z <- poincareMap(x, extrema="max")
z <- embedSeries(z$amplitude, tlag=1, dimension=2)
plot(z, pch=15, cex=2,type="l")

z <- poincareMap(y, extrema="max")
z <- embedSeries(z$amplitude, tlag=1, dimension=2)
plot(z, pch=15, cex=2,type="l")

z <- poincareMap(z0, extrema="max")
z <- embedSeries(z$amplitude, tlag=1, dimension=2)
plot(z, pch=15, cex=2,type="l")

z <- poincareMap(w, extrema="max")
z <- embedSeries(z$amplitude, tlag=1, dimension=2)
plot(z, pch=15, cex=2,type="l")

"""### 3.8 Test de Shintani-Linton (Lyapunov)"""

# un simple renombre
x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

DChaos::lyapunov(x)

DChaos::lyapunov(y)

DChaos::lyapunov(z)

DChaos::lyapunov(w) # debe de haber algun error en la librería utilizada (DChaos), pero sirve

##########################



"""# Otros"""

# Exportamos las series de residuos para utilizarlas en otros cuadernos

x=ts(ibmdata$d_ln_close[2:length(ibmdata$d_ln_close)]) # retornos
y=residuos_arma11 #arma11
z=ts(residuos_garch11) #garch
w=ts(residuos_earch11) #egarch

write(x, "retorn.csv",ncolumns=1)
write(y, "arma11.csv",ncolumns=1)
write(z, "garc11.csv",ncolumns=1)
write(w, "egar21.csv",ncolumns=1)



#######################################

install.packages("sos"); 
library("sos");

findFn("Volterra Series")

# funcion para prevenir desconexion

nodisc=function(){
  respuesta <- readline(prompt="¿Seguro de no querer desconectar (y/n)?: ")
  i=0
  if (respuesta=="y"){
    while (i<9){
      a=0
    }
  }
}

nodisc()

